...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function addPostToTable(postId, post) {
        ...
        if (post.latLng) {
            ...
            const aerialViewIframe = createAerialView(post.latLng);
            latLngCell.appendChild(aerialViewIframe);
        }
    ...
    // Function to generate and display the small Aerial View video
    function createAerialView(latLng) {
        const iframe = document.createElement('iframe');
        const aerialViewURL = `https://www.google.com/maps/embed/v1/aerial?key=AIzaSyDO-oSBQ_9smffiOepTecZJyXlydEsfpgs&center=${latLng}&zoom=18&maptype=satellite`;
        iframe.src = aerialViewURL;
        iframe.style.width = '150px';
        iframe.style.height = '100px';
        iframe.style.cursor = 'pointer';
        iframe.addEventListener('click', function () {
            openLargeVideo(aerialViewURL);
        });
        return iframe;
    }

    // Function to open large video in modal
    function openLargeVideo(videoURL) {
